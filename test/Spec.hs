...
main = hspec $ do
  describe "Sauna.options" $ do
    ...
    it "handles solo letters correcly" $ do
      options
          (State
            [ ( Word (Quintuple (A, B, C, D, E))
              , Response (Quintuple (Green, Yellow, Black, Black, Black))
              )
            ]
          )
        `shouldBe` Quintuple
    ...
    it "handles double letters correcrly" $ do
      options
          (State
            [ ( Word (Quintuple (A, A, B, B, B))
              , Response (Quintuple (Yellow, Black, Black, Black, Black))
              )
            ]
          )
        `shouldBe` Quintuple
  ...
  describe "Sauna.check" $ do
    ...
    it
        "return Yellow only for one occurence of the same letter on diferent positions"
      $ do
          check (read "ABCDE") (read "BGBBB")
            `shouldBe` (read "YBBBB" :: Response)
          check (read "ABCDE") (read "FCHCC")
            `shouldBe` (read "BYBBB" :: Response)
          check (read "ABCDE") (read "FGDID")
            `shouldBe` (read "BBYBB" :: Response)
